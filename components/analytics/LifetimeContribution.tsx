...
export function LifetimeContribution({ year }: { year?: number }) {
    const { data: analytics, isLoading } = useAnalytics(year)
    const [data, setData] = useState<any[]>([])

    useEffect(() => {
        if (analytics?.lifetimeContributionPie) {
            setData(analytics.lifetimeContributionPie)
        }
    }, [analytics])
    ...
    return (
        <GraphContainer
            ...
            description={`Distribution of total completions across all goals for ${year || 'all time'}`}
            fullWidth
            tall
        >
            {isLoading ? (
                ...
                <div className="flex items-center justify-center h-full text-neutral-500 text-sm">
                    No lifetime contribution data yet.
                </div>
            ) : (
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                            ...
                            <Tooltip
                                ...
                                formatter={(value) => [`${value} Total Completions`, 'Volume']}
                            />
                            <Legend
                                layout="vertical"
                                align="right"
...
export default memo(LifetimeContribution)
