            ...
            dispose() {
                try {
                    ...
                    if (Common.renderer) {
                        ...
                        // DISPOSE RESOURCES (GPU Memory Cleanup)
                        if (this.output) {
                            if (this.output.scene) {
                                this.output.scene.traverse((object: any) => {
                            ...
                            if (this.output.simulation) {
                                ...
                                if (sim.advection?.material) sim.advection.material.dispose();
                                if (sim.advection?.geometry) sim.advection.geometry.dispose();
                                if (sim.externalForce?.material) sim.externalForce.material.dispose();
                                if (sim.externalForce?.geometry) sim.externalForce.geometry.dispose();
                                if (sim.viscous?.material) sim.viscous.material.dispose();
                                if (sim.viscous?.geometry) sim.viscous.geometry.dispose();
                                if (sim.divergence?.material) sim.divergence.material.dispose();
                                if (sim.divergence?.geometry) sim.divergence.geometry.dispose();
                                if (sim.poisson?.material) sim.poisson.material.dispose();
                                if (sim.poisson?.geometry) sim.poisson.geometry.dispose();
                                if (sim.pressure?.material) sim.pressure.material.dispose();
                                if (sim.pressure?.geometry) sim.pressure.geometry.dispose();
                            }
                        }
